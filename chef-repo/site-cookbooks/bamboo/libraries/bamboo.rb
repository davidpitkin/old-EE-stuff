  ...
  def self.settings(node)
    begin
      if Chef::Config[:solo]
        ...
        begin
          private_key = Chef::EncryptedDataBagItem.load_secret("#{node["ee-bamboo"]["secret_file"]}")
          settings = Chef::EncryptedDataBagItem.load("bamboo","bamboo", private_key)[node.chef_environment]
          Chef::Log.info("Loaded" + settings.inspect)
          Chef::Log.info("in environment #{node.chef_environment}")
        rescue
